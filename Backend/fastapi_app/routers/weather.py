...
async def sync_all():
    try:
        import httpx
        today = date.today()
        start = today - timedelta(days=cfg.archive_days)
        ...
        async with pool.acquire() as conn:
            for i, d in enumerate(arch_daily.get("time", [])):
                ...
                await conn.execute("""
                    ...
                    DO UPDATE SET temp_max=$5,temp_min=$6,temp_mean=$7,
                      humidity_mean=$8,precipitation_sum=$9,
                      wind_speed_max=$10,updated_at=NOW()
                """, d, cfg.location_key, cfg.latitude, cfg.longitude,
                     tmax, tmin, tmean, hum, prec or 0, wind or 0)
                arch_rows += 1
            ...
            for i, d in enumerate(fore_daily.get("time", [])):
                ...
                await conn.execute("""
                    ...
                    DO UPDATE SET temp_max=$4,temp_min=$5,temp_mean=$6,
                      humidity_mean=$7,precipitation_sum=$8,
                      wind_speed_max=$9,updated_at=NOW()
                """, d, cfg.latitude, cfg.longitude,
                     tmax, tmin, tmean, hum, prec or 0, wind or 0)
                fore_rows += 1

        return {"ok": True, "message": f"Sync complete — archive: {arch_rows} rows, forecast: {fore_rows} rows"}
    except Exception as e:
        raise HTTPException(500, str(e))
...
async def get_archive(limit: int = Query(60, le=365)):
    try:
        ...
        rows = await pool.fetch(
            """SELECT date,location_key,latitude,longitude,temp_max,temp_min,temp_mean,
                      humidity_mean,precipitation_sum,wind_speed_max,data_source,updated_at
               FROM daily_weather WHERE location_key=$1 AND data_source='archive'
               ORDER BY date DESC LIMIT $2""",
            cfg.location_key, limit,
        )
    ...
    try:
        ...
        rows = await pool.fetch(
            """SELECT date,latitude,longitude,temp_max,temp_min,temp_mean,
                      humidity_mean,precipitation_sum,wind_speed_max,updated_at
               FROM daily_weather_forecast WHERE latitude=$1 AND longitude=$2
               ORDER BY date ASC""",
