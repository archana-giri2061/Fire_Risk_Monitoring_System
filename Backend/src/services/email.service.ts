...
export function buildFireAlertHtml(args: BuildEmailArgs): string {
  ...
  const dayRows = highDays
    .map((d) => {
      ...
      return `
        <tr>
          <td style="padding:10px 16px;border-bottom:1px solid #e5e7eb;font-weight:600;">
            ${d.date}
          </td>
        ...
        <tr>
          <td style="background:${style.border};padding:32px 40px;text-align:center;">
            ...
            <p style="margin:8px 0 0;color:rgba(255,255,255,0.88);font-size:15px;">
              ${style.icon} ${worstLabel} Risk Level Detected
            </p>
          </td>
        </tr>
        ...
        <tr>
          <td style="background:${style.bg};padding:16px 40px;border-bottom:2px solid ${style.border};">
            <table width="100%" cellpadding="0" cellspacing="0">
              <tr>
                <td>
                  <span style="font-size:13px;color:${style.color};font-weight:600;text-transform:uppercase;">
                     Monitored Location
                  </span><br/>
                  <span style="font-size:18px;font-weight:800;color:${style.color};">
                    ${location}
                  </span>
                </td>
        ...
        <tr>
          <td style="padding:32px 40px;">
            <p style="margin:0 0 20px;color:#111827;font-size:15px;line-height:1.6;">
              ...
              in the upcoming 7-day forecast. Immediate precautionary measures are recommended.
            </p>
            ...
            <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:20px 24px;margin-bottom:28px;">
              <h3 style="margin:0 0 12px;font-size:15px;color:#111827;font-weight:700;">
                 Recommended Actions
              </h3>
            ...
            <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:16px;">
              <p style="margin:0;font-size:13px;color:#1e40af;line-height:1.6;">
                <strong> About this alert:</strong> Automatically generated by the
                Wildfire Risk Monitoring System using XGBoost ML predictions based on
                Open-Meteo weather forecast data. Alert threshold: <strong>${threshold}+</strong>.
              </p>
            </div>
          </td>
        </tr>
...
export async function sendFireAlert(args: SendAlertArgs) {
  ...
  if (!config.smtp.to) {
    throw new Error("ALERT_TO_EMAIL is not set in .env");
  }
  ...
  const info = await transporter.sendMail({
    from:    `" Fire Risk Monitor" <${config.smtp.from || config.smtp.user}>`,
    to:      allTo.join(", "),
  ...
  return { messageId: info.messageId, recipients: allTo };
}
