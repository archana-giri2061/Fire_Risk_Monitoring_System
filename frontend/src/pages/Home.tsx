        ...
        <aside style={{
          ...
          <nav style={{ flex: 1, padding: "16px 10px", display: "flex", flexDirection: "column", gap: 4 }}>
            {links.map(lk => {
              ...
              return (
                <Link key={lk.to} to={lk.to} onClick={() => setMobileOpen?.(false)}
                  style={{ display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 14, background: active ? "linear-gradient(135deg,rgba(241,178,74,0.22),rgba(241,178,74,0.08))" : "transparent", border: active ? "1px solid rgba(241,178,74,0.25)" : "1px solid transparent", color: active ? "#F1B24A" : "rgba(255,255,255,0.7)", fontWeight: active ? 700 : 500, fontSize: 15, textDecoration: "none" }}>
...
function StatCard({ icon, label, value, unit, color, sub, trend }: { icon: React.ReactNode; label: string; value: string | number; unit?: string; color: string; sub?: string; trend?: "up" | "down"; }) {
  return (
    <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "16px 18px", transition: "all 0.25s" }}>
      ...
      <div style={{ fontSize: 26, fontWeight: 900, color, lineHeight: 1, marginBottom: 4 }}>
        {value}<span style={{ fontSize: 13, fontWeight: 600, marginLeft: 3, color: "rgba(255,255,255,0.5)" }}>{unit}</span>
      </div>
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "transparent" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{ flex: 1, padding: isMobile ? "16px" : "24px", display: "flex", flexDirection: "column", gap: isMobile ? 16 : 22, overflowY: "auto" }}>
          ...
          {error && (
            <div style={{ padding: "12px 16px", borderRadius: 14, display: "flex", alignItems: "center", gap: 10, background: "rgba(255,77,77,0.12)", border: "1px solid rgba(255,77,77,0.25)", color: "#ff9999", fontSize: 13 }}>
          ...
          {worstPred && (
            <div style={{ padding: isMobile ? "14px 16px" : "14px 20px", borderRadius: 16, display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", background: `${RISK_BG[worstPred.risk_label]}`, border: `1px solid ${RISK_COLOR[worstPred.risk_label]}40`, gap: 12, flexDirection: isMobile ? "column" : "row" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  <span style={{ fontWeight: 700, color: RISK_COLOR[worstPred.risk_label], fontSize: isMobile ? 14 : 15 }}>{RISK_ICON[worstPred.risk_label]} {worstPred.risk_label} Risk</span>
                  <span style={{ color: "rgba(255,255,255,0.55)", fontSize: 12, marginLeft: 8 }}>predicted for {worstPred.date} · {Math.round(worstPred.risk_probability * 100)}%</span>
                </div>
              </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: isMobile ? 10 : 16 }}>
            <StatCard icon={<Thermometer size={18} />} label="Temperature" value={ov?.temperature?.toFixed(1) ?? "--"} unit="°C"   color="#ff8c42" sub="Current" trend="up" />
            <StatCard icon={<Droplets size={18} />}    label="Humidity"    value={ov?.humidity?.toFixed(0) ?? "--"}     unit="%"    color="#60a5fa" sub="Relative" />
            <StatCard icon={<Wind size={18} />}        label="Wind"        value={ov?.windSpeed?.toFixed(1) ?? "--"}    unit="km/h" color="#9DC88D" sub="Max speed" trend="up" />
            <StatCard icon={<CloudRain size={18} />}   label="Rainfall"    value={ov?.rainfall?.toFixed(1) ?? "--"}     unit="mm"   color="#a78bfa" sub="Sum" />
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 2fr", gap: isMobile ? 14 : 16 }}>
                ...
                return (
                  <>
                    <div style={{ textAlign: "center", padding: "16px 0" }}>
                      ...
                      <div style={{ fontSize: isMobile ? 36 : 46, fontWeight: 900, color: rColor, lineHeight: 1, textShadow: `0 0 30px ${rColor}50` }}>{rLabel}</div>
            ...
            <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "18px 20px" }}>
              ...
              {preds.length === 0 ? (
                <div style={{ textAlign: "center", padding: "28px 0", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>No predictions yet</div>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(preds.length, 7)},1fr)`, gap: isMobile ? 6 : 8 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1.5fr 1fr", gap: isMobile ? 14 : 16 }}>
            ...
            <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "18px 20px" }}>
              ...
              {data?.trends && data.trends.length > 0 ? (
                ...
                <div style={{ textAlign: "center", padding: "28px 0", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>No data. Run POST /api/weather/sync-all</div>
              )}
            </div>
          ...
          <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "18px 20px", overflowX: "auto" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: isMobile ? 500 : "auto" }}>
              ...
              <tbody>
                ...
                }) ?? <tr><td colSpan={6} style={{ textAlign: "center", padding: "28px", color: "rgba(255,255,255,0.3)", fontSize: 13 }}>No readings yet</td></tr>}
              </tbody>
            </table>
          </div>
