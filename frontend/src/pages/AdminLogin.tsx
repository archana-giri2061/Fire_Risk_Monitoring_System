    ...
    <div>
      <div style={{ position: "relative", marginBottom: 14 }}>
        ...
        <input
          ...
          autoFocus
          style={{ width: "100%", padding: "10px 40px 10px 36px", borderRadius: 12, background: "rgba(255,255,255,0.06)", border: error ? "1px solid rgba(255,77,77,0.5)" : "1px solid rgba(255,255,255,0.12)", color: "#fff", fontSize: 14, outline: "none", boxSizing: "border-box" }}
        />
        <button
          onClick={() => setShow(!show)}
          style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: "rgba(255,255,255,0.35)", cursor: "pointer", padding: 4 }}
        >
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button
          onClick={submit}
          disabled={loading}
          style={{ flex: 1, padding: "10px", borderRadius: 12, background: "rgba(241,178,74,0.15)", border: "1px solid rgba(241,178,74,0.3)", color: "#F1B24A", fontWeight: 700, fontSize: 14, cursor: loading ? "wait" : "pointer" }}
        >
          {loading ? "Verifying…" : "Login"}
        </button>
        <button
          onClick={onCancel}
          style={{ padding: "10px 16px", borderRadius: 12, background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "rgba(255,255,255,0.5)", fontSize: 14, cursor: "pointer" }}
        >
          Cancel
        </button>
      </div>
