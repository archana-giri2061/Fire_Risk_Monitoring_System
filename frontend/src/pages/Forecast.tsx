      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        ...
        <div style={{ padding: isMobile ? "12px 16px" : "14px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", borderBottom: "1px solid rgba(255,255,255,0.07)", background: "rgba(8,22,18,0.6)", backdropFilter: "blur(14px)", position: "sticky", top: 0, zIndex: 10, gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={fetchData} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", borderRadius: 999, background: "rgba(157,200,141,0.12)", border: "1px solid rgba(157,200,141,0.25)", color: "#9DC88D", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              <RefreshCw size={12} />{!isMobile && " Refresh"}
            </button>
            <button onClick={triggerAlert} disabled={runningAlert} style={{ display: "flex", alignItems: "center", gap: 6, padding: "8px 12px", borderRadius: 999, background: "rgba(255,140,66,0.15)", border: "1px solid rgba(255,140,66,0.3)", color: "#ff8c42", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>
              <Bell size={12} style={{ animation: runningAlert ? "spin 1s linear infinite" : "none" }} />
              {isMobile ? "" : runningAlert ? "Sending…" : "Send Alert"}
            </button>
          </div>
        </div>
        ...
        <div style={{ flex: 1, padding: isMobile ? "16px" : "24px", display: "flex", flexDirection: "column", gap: isMobile ? 14 : 22, overflowY: "auto" }}>
          ...
          {alertMsg && (
            <div style={{ padding: "12px 16px", borderRadius: 14, background: alertMsg.startsWith("✅") ? "rgba(157,200,141,0.12)" : alertMsg.startsWith("ℹ") ? "rgba(241,178,74,0.12)" : "rgba(255,77,77,0.12)", border: `1px solid ${alertMsg.startsWith("✅") ? "rgba(157,200,141,0.3)" : alertMsg.startsWith("ℹ") ? "rgba(241,178,74,0.3)" : "rgba(255,77,77,0.3)"}`, color: "#fff", fontSize: 13 }}>{alertMsg}</div>
          )}
          ...
          {worstLabel && (
            <div style={{ padding: isMobile ? "14px 16px" : "18px 24px", borderRadius: 18, background: RISK_BG[worstLabel], border: `1px solid ${RISK_COLOR[worstLabel]}40`, display: "flex", alignItems: isMobile ? "flex-start" : "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ width: 44, height: 44, borderRadius: 14, background: `${RISK_COLOR[worstLabel]}20`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  <Flame size={22} color={RISK_COLOR[worstLabel]} />
                </div>
                <div>
                ...
                {["Low", "Moderate", "High", "Extreme"].map(lbl => {
                  ...
                  return (
                    <div key={lbl} style={{ textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 900, color: RISK_COLOR[lbl] }}>{cnt}</div>
          ...
          {preds.length === 0 && (
            <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "48px 20px", textAlign: "center" }}>
              ...
              <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>No forecast data yet</div>
              <div style={{ fontSize: 12, color: "rgba(255,255,255,0.4)", marginTop: 6 }}>Run POST /api/ml/predict-forecast</div>
            </div>
          )}
                    ...
                    <div style={{ padding: isMobile ? "14px 16px" : "18px 22px", display: "flex", alignItems: "center", gap: isMobile ? 12 : 16, cursor: "pointer" }} onClick={() => setExpandedDay(expanded ? null : i)}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                          ...
                          {isToday && <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 999, background: "rgba(157,200,141,0.2)", color: "#9DC88D", border: "1px solid rgba(157,200,141,0.35)" }}>TODAY</span>}
                        ...
                        {isMobile && <div style={{ marginTop: 8, display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    {expanded && (
                      <div style={{ padding: isMobile ? "0 16px 16px" : "0 22px 20px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                        <div style={{ paddingTop: 16, display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "1fr 1fr 1fr", gap: 10 }}>
                          <div style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginBottom: 6, textTransform: "uppercase" }}>Risk Score</div>
                          ...
                          <div style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)" }}>
                            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginBottom: 6, textTransform: "uppercase" }}>Risk Category</div>
                            <div style={{ fontSize: 16, fontWeight: 800, color: col }}>{RISK_ICON[p.risk_label]} {p.risk_label}</div>
                            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>Code: {p.risk_code}</div>
                          </div>
                          <div style={{ padding: "14px", borderRadius: 14, background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.07)", gridColumn: isMobile ? "1 / -1" : "auto" }}>
                            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", marginBottom: 6, textTransform: "uppercase" }}>ML Model</div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: "#fff" }}>{p.model_name || "XGBoost"}</div>
                            <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 4 }}>Multi-class · 6 features</div>
                          </div>
                        </div>
                        {["High", "Extreme"].includes(p.risk_label) && (
                          <div style={{ marginTop: 12, padding: "12px 16px", borderRadius: 14, background: `${col}10`, border: `1px solid ${col}25`, display: "flex", alignItems: "flex-start", gap: 10 }}>
                            ...
                            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.7)" }}>
                              ...
                              {p.risk_label === "Extreme" ? "Extreme fire danger — avoid all open burning, deploy fire watch teams." : "High fire danger — restrict burning, increase patrol frequency."}
                            </div>
                          </div>
                        )}
                      </div>
            ...
            <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)", borderRadius: 20, padding: "18px 20px" }}>
              ...
              <div style={{ display: "flex", height: 28, borderRadius: 8, overflow: "hidden", gap: 2 }}>
                {["Low", "Moderate", "High", "Extreme"].map(lbl => {
                  ...
                  return <div key={lbl} style={{ flex: pct, background: RISK_COLOR[lbl], display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: "#1d241e", minWidth: pct > 8 ? "auto" : 0 }}>{pct > 12 ? `${cnt}d` : ""}</div>;
                })}
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 14, marginTop: 10 }}>
                {["Low", "Moderate", "High", "Extreme"].map(lbl => (
                  <div key={lbl} style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "rgba(255,255,255,0.5)" }}>
            ...
            {history.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 480 }}>
                  <thead>
                    <tr>{["Date", "Location", "Risk", "Message", "Sent At"].map(h => (
                    ...
                    {history.map((row, i) => {
                      ...
                      return (
                        <tr key={i} style={{ borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                          <td style={{ padding: "10px", fontSize: 12, color: "rgba(255,255,255,0.6)" }}>{row.alert_date}</td>
                          <td style={{ padding: "10px", fontSize: 12, color: "rgba(255,255,255,0.6)" }}>{row.location_key}</td>
                          <td style={{ padding: "10px" }}><span style={{ padding: "3px 8px", borderRadius: 999, fontSize: 11, fontWeight: 700, background: `${col}18`, color: col, border: `1px solid ${col}30` }}>{row.risk_label}</span></td>
                          <td style={{ padding: "10px", fontSize: 12, color: "rgba(255,255,255,0.5)", maxWidth: 200 }}>{row.message}</td>
                          <td style={{ padding: "10px", fontSize: 11, color: "rgba(255,255,255,0.35)", whiteSpace: "nowrap" }}>{new Date(row.created_at).toLocaleDateString("en", { month: "short", day: "numeric" })}</td>
                        </tr>
                      );
                    })}
                  </tbody>
